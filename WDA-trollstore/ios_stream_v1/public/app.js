...
const CONTROL_MODE_REALTIME = 'realtime';
const CONTROL_MODE_HTTP_POINT_ARRAY = 'http-point-array';
const CONTROL_MODE_HTTP_SWIPE = 'http-swipe';
const CONTROL_MODE_ALIASES = {
  realtime: CONTROL_MODE_REALTIME,
  socket: CONTROL_MODE_REALTIME,
  auto: CONTROL_MODE_HTTP_POINT_ARRAY,
  http: CONTROL_MODE_HTTP_POINT_ARRAY,
  'http-wda': CONTROL_MODE_HTTP_POINT_ARRAY,
  pointarray: CONTROL_MODE_HTTP_POINT_ARRAY,
  'point-array': CONTROL_MODE_HTTP_POINT_ARRAY,
  'http-pointarray': CONTROL_MODE_HTTP_POINT_ARRAY,
  'http-point-array': CONTROL_MODE_HTTP_POINT_ARRAY,
  swipe: CONTROL_MODE_HTTP_SWIPE,
  swip: CONTROL_MODE_HTTP_SWIPE,
  'http-swipe': CONTROL_MODE_HTTP_SWIPE,
  'http-swip': CONTROL_MODE_HTTP_SWIPE,
};
const CONTROL_MODES_FALLBACK = [
  {id: CONTROL_MODE_REALTIME, label: 'Realtime socket', enabled: true, reachable: true},
  {id: CONTROL_MODE_HTTP_POINT_ARRAY, label: 'HTTP PointArray', enabled: true, reachable: true},
  {id: CONTROL_MODE_HTTP_SWIPE, label: 'HTTP Swipe', enabled: true, reachable: true},
];
...
function bindUi() {
  ...
  el.controlModeSelect.addEventListener('change', () => {
    const mode = normalizeControlModeId(el.controlModeSelect.value) || CONTROL_MODE_HTTP_POINT_ARRAY;
...
async function loadControlModes() {
  ...
  if (preferredMode === CONTROL_MODE_REALTIME && upstream.realtimeReachable) {
    setControlState('socket available');
  } else if (preferredMode === CONTROL_MODE_REALTIME) {
    setControlState('socket unavailable');
  } else if (preferredMode === CONTROL_MODE_HTTP_POINT_ARRAY) {
    setControlState('http pointArray');
  } else {
    setControlState('http swipe');
  }
...
function choosePreferredControlMode(selectedMode, modes) {
  ...
  return modes.find((item) => item.id === CONTROL_MODE_HTTP_POINT_ARRAY && item.enabled)?.id
    || modes.find((item) => item.id === CONTROL_MODE_HTTP_SWIPE && item.enabled)?.id
    || modes.find((item) => item.enabled)?.id
    || modes[0]?.id
    || CONTROL_MODE_HTTP_POINT_ARRAY;
}
...
function isPointArraySwipeEnabled() {
  return getSelectedControlMode() === CONTROL_MODE_HTTP_POINT_ARRAY;
}
...
async function applySelectedControlMode(force = false) {
  ...
  persistRequestedControlMode(mode);
  setControlModeLabel(mode);
  if (mode !== CONTROL_MODE_REALTIME) {
    closeRealtimeControlClient();
    setControlState(mode === CONTROL_MODE_HTTP_POINT_ARRAY ? 'http pointArray' : 'http swipe');
    return;
  }
...
function shouldUseRealtimeTouchStream() {
  return getSelectedControlMode() === CONTROL_MODE_REALTIME;
}
...
async function sendControlCommand(command, httpFallback, options = {}) {
  ...
  if (mode !== CONTROL_MODE_REALTIME || pinnedTransport === 'http') {
    if (!allowHttpFallback) {
      const err = new Error('Realtime touch stream requires Realtime socket mode');
      err.sent = false;
      throw err;
    }
    setControlState(mode === CONTROL_MODE_HTTP_SWIPE ? 'http swipe' : 'http pointArray');
  ...
  try {
    ...
    setControlState(mode === CONTROL_MODE_HTTP_SWIPE ? 'http swipe fallback' : 'http pointArray fallback');
    const result = await httpFallback();
    return result && typeof result === 'object'
      ? {...result, transport: result.transport || 'http'}
      : result;
...
function isLiveTouchCommand(command) {
  const type = String(command?.type || '').trim().toLowerCase().replace(/[-_]/g, '');
  return (
    type === 'touchdown' ||
    type === 'touchmove' ||
    type === 'touchup' ||
...
function createRealtimeControlClient(endpoint) {
  ...
  const scheduleReconnect = () => {
    if (!shouldReconnect || reconnectTimer || getSelectedControlMode() !== CONTROL_MODE_REALTIME) {
      return;
    }
