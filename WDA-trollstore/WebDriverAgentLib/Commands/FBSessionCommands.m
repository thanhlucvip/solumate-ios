...
+ (NSArray *)routes
{
  ...
  @[
    ...
    [[FBRoute GET:@"/wda/apps/installed"] respondWithTarget:self action:@selector(handleGetInstalledAppsList:)],
    [[FBRoute GET:@""] respondWithTarget:self action:@selector(handleGetActiveSession:)],
    [[FBRoute DELETE:@""] respondWithTarget:self action:@selector(handleDeleteSession:)],
    [[FBRoute GET:@"/status"].withoutSession respondWithTarget:self action:@selector(handleGetStatus:)],

    // Health check might modify simulator state.
    [[FBRoute GET:@"/wda/healthcheck"] respondWithTarget:self action:@selector(handleGetHealthCheck:)],
...
+ (BOOL)isApplicationInstalled:(NSString *)bundleId
{
  if (nil == bundleId || bundleId.length == 0) {
    return NO;
  }
  for (NSDictionary<NSString *, id> *appInfo in XCUIApplication.fb_allInstalledAppsInfo) {
    NSString *installedBundleId = appInfo[@"bundleId"];
    if ([installedBundleId isEqualToString:bundleId]) {
      return YES;
    }
  }
  return NO;
...
+ (id<FBResponsePayload>)handleAppLaunchWithoutSession:(FBRouteRequest *)request
{
  ...
  if (![bundleId isKindOfClass:NSString.class] || bundleId.length == 0) {
    return FBResponseWithStatus([FBCommandStatus invalidArgumentErrorWithMessage:@"The 'bundleId' argument must be provided"
                                                                       traceback:nil]);
  }
  if (![self.class isApplicationInstalled:bundleId]) {
    return FBResponseWithStatus([FBCommandStatus invalidArgumentErrorWithMessage:[NSString stringWithFormat:@"The application '%@' is not installed on the device under test", bundleId]
                                                                       traceback:nil]);
  }

  XCUIApplication *app = [[XCUIApplication alloc] initWithBundleIdentifier:bundleId];
  if ([request.arguments[@"arguments"] isKindOfClass:NSArray.class]) {
    app.launchArguments = (NSArray<NSString *> *)request.arguments[@"arguments"];
  }
  if ([request.arguments[@"environment"] isKindOfClass:NSDictionary.class]) {
    app.launchEnvironment = (NSDictionary<NSString *, NSString *> *)request.arguments[@"environment"];
  }
  if ([request.arguments[@"shouldWaitForQuiescence"] isKindOfClass:NSNumber.class]) {
    app.fb_shouldWaitForQuiescence = [request.arguments[@"shouldWaitForQuiescence"] boolValue];
  }
  @try {
    [app launch];
  } @catch (NSException *e) {
    return FBResponseWithStatus([FBCommandStatus unknownErrorWithMessage:e.reason traceback:nil]);
  }
  return FBResponseWithOK();
}

+ (id<FBResponsePayload>)handleAppActivateWithoutSession:(FBRouteRequest *)request
{
  NSString *bundleId = request.arguments[@"bundleId"];
  if (![bundleId isKindOfClass:NSString.class] || bundleId.length == 0) {
    return FBResponseWithStatus([FBCommandStatus invalidArgumentErrorWithMessage:@"The 'bundleId' argument must be provided"
...
+ (id<FBResponsePayload>)handleAppTerminateWithoutSession:(FBRouteRequest *)request
{
  ...
  @try {
    ...
    [FBLogger logFmt:@"Cannot terminate application '%@': %@", bundleId, e.reason];
    return FBResponseWithObject(@NO);
  }
}

+ (id<FBResponsePayload>)handleAppStateWithoutSession:(FBRouteRequest *)request
{
  NSString *bundleId = request.arguments[@"bundleId"];
  if (![bundleId isKindOfClass:NSString.class] || bundleId.length == 0) {
    return FBResponseWithStatus([FBCommandStatus invalidArgumentErrorWithMessage:@"The 'bundleId' argument must be provided"
  ...
  XCUIApplication *app = [[XCUIApplication alloc] initWithBundleIdentifier:bundleId];
  return FBResponseWithObject(@(app.state));
}
...
+ (id<FBResponsePayload>)handleGetInstalledAppsList:(FBRouteRequest *)request
{
  return FBResponseWithObject([XCUIApplication fb_allInstalledAppsInfo]);
}
...
+ (id<FBResponsePayload>)handleGetStatus:(FBRouteRequest *)request
{
  ...
  NSString *productBundleIdentifier = @"solumate.driver.automation";
...
+ (id<FBResponsePayload>)handleGetSettings:(FBRouteRequest *)request
{
  ...
  return FBResponseWithObject(
    @{
      ...
      FB_SETTING_DEFAULT_ACTIVE_APPLICATION: session.defaultActiveApplication ?: @"",
      ...
      FB_SETTING_DEFAULT_ALERT_ACTION: session.defaultAlertAction ?: @"",
...
+ (id<FBResponsePayload>)handleGetSettingsWithoutSession:(FBRouteRequest *)request
{
...
+ (id<FBResponsePayload>)handleSetSettingsWithoutSession:(FBRouteRequest *)request
{
  return [self handleSetSettings:request];
}
