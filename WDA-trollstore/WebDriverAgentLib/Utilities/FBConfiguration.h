...
/**
 The IP address that raw TCP stream servers should bind to on launch.
 Defaults to the HTTP binding address.
 */
+ (NSString * _Nullable)streamBindingIPAddress;
...
/**
 The port number where the raw H264 broadcaster is supposed to run.
 Returns -1 when H264 streaming is disabled.
 */
+ (NSInteger)h264ServerPort;

/**
 The port number where the realtime control socket is supposed to run.
 Returns 8003 by default when enabled.
 */
+ (NSInteger)realtimeControlPort;

/**
 Whether the realtime control socket should be started.
 Enabled by default for TrollStore/manual-launch builds.
 */
+ (BOOL)realtimeControlEnabled;

/**
 The IP address that the realtime control socket should bind to on launch.
 Defaults to the stream binding address.
 */
+ (NSString * _Nullable)realtimeControlBindingIPAddress;

/**
 Exact CORS origin to allow. Returns nil by default, which disables CORS headers.
 */
+ (NSString * _Nullable)allowedCORSOrigin;

/**
 Whether HTTP and stream requests must present a WDA auth token.
 Defaults to NO unless WDA_AUTH_TOKEN/WDA_REQUIRE_AUTH is configured or the
 HTTP/stream listeners are explicitly bound to a non-loopback address.
 */
+ (BOOL)requiresAuthentication;

/**
 Whether an auth token has been configured.
 */
+ (BOOL)isAuthenticationConfigured;

/**
 Validates HTTP auth headers and query parameters against WDA_AUTH_TOKEN.
 */
+ (BOOL)isRequestAuthorizedWithHeaders:(NSDictionary *)headers
                       queryParameters:(NSDictionary *)queryParameters;

/**
 Validates the first packet sent to a raw stream server.
 */
+ (BOOL)isStreamHandshakeAuthorized:(NSData *)data;

/**
 Maximum JSON/body bytes accepted by the WDA HTTP routing layer.
 */
+ (NSUInteger)maximumHTTPRequestBodySize;

/**
 Maximum decoded bytes accepted by the push file/media commands.
 */
