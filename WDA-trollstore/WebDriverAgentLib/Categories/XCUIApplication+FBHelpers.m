...
+ (NSArray<NSDictionary<NSString *, id> *> *)fb_allInstalledAppsInfo
{
  @try {
    LSApplicationWorkspace *workspace = [LSApplicationWorkspace defaultWorkspace];
    if (nil == workspace) {
      [FBLogger log:@"LSApplicationWorkspace defaultWorkspace returned nil"];
      return @[];
    }
    NSArray *allApplications = [workspace allApplications];
    if (nil == allApplications) {
      [FBLogger log:@"allApplications returned nil"];
      return @[];
    }
    [FBLogger logFmt:@"Found %lu applications using allApplications",
                     (unsigned long)allApplications.count];
    NSMutableArray<NSDictionary<NSString *, id> *> *result = [NSMutableArray array];
    for (id app in allApplications) {
      @autoreleasepool {
        @try {
          NSString *bundleIdentifier = [app valueForKey:@"bundleIdentifier"];
          if (nil == bundleIdentifier || bundleIdentifier.length == 0) {
            continue;
          }
          NSMutableDictionary<NSString *, id> *appInfo = [NSMutableDictionary dictionary];
          appInfo[@"bundleId"] = bundleIdentifier;

          NSString *localizedName = [app valueForKey:@"localizedName"];
          if (localizedName.length > 0) {
            appInfo[@"name"] = localizedName;
          }

          NSString *applicationType = [app valueForKey:@"applicationType"];
          if (applicationType.length > 0) {
            appInfo[@"applicationType"] = applicationType;
          }

          NSString *shortVersionString = [app valueForKey:@"shortVersionString"];
          if (shortVersionString.length > 0) {
            appInfo[@"version"] = shortVersionString;
          }
          [result addObject:appInfo.copy];
        } @catch (NSException *innerException) {
          [FBLogger logFmt:@"Error extracting app info: %@", innerException.reason];
        }
      }
    }
    return result.copy;
  } @catch (NSException *e) {
    [FBLogger logFmt:@"Error getting installed apps: %@", e.reason];
    return @[];
  }
}
