...
/**
 Returns screenshot with optional crop and scale transform.

 @param error The screenshot capture error.
 @param rect Crop rectangle in screen point coordinates. Pass CGRectZero to keep original bounds.
 @param scale Optional scale factor in range (0, 1]. Pass nil to keep original size.
 @param transformError If there is a transform error, upon return contains an NSError object that describes the problem.
 @return Device screenshot as PNG-encoded data or nil in case of failure.
 */
- (nullable NSData *)fb_screenshotWithError:(NSError * _Nullable __autoreleasing *)error
                                       rect:(CGRect)rect
...
/**
 Returns device's current actual wifi ip4 address.
 It is equivalent to fb_wifiIPAddress and preserved for backward compatibility.
 */
...
/**
 Performs a coordinate tap gesture on the active application.
 */
...
/**
 Performs a swipe gesture between two points on the active application.
 */
...
/**
 Performs a multi-segment swipe for an array of points.
 Supports both dictionary points (`{x,y,t}`) and array points (`[x,y,t]`).
 */
- (BOOL)fb_qx9:(NSArray *)items;

/**
 Starts a stateful realtime touch at the given device coordinate.
 */
...
/**
 Updates the currently active realtime touch.
 */
...
/**
 Releases the currently active realtime touch.
 */
...
/**
 Cancels/releases any active realtime touch.
 */
- (void)fb_realtimeTouchCancel;

/**
 Returns private HID injector diagnostics. If dispatchProbe is YES a short
 down/up probe is attempted at the requested coordinate.
 */
