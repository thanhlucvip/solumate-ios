...
@property (readonly, nullable, nonatomic, copy) NSString *interface;
@end
...
{
  return [self initWithPort:port interface:nil];
}

- (instancetype)initWithPort:(uint16_t)port interface:(NSString *)interface
{
  if ((self = [super init])) {
...
{
  if (![self.listeningSocket acceptOnInterface:self.interface port:self.port error:error]) {
    return NO;;
  }
...
{
  [self.delegate didClient:sock didReadData:data];
}
