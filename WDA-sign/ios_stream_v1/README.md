...
Dự án này chọn hướng **WebDriverAgent MJPEG** thay vì `quicktime_video_hack + ws-qvh` làm bản chạy thực dụng đầu tiên trên Ubuntu:

- dễ ghép vào Node.js hơn
- không cần tự remux raw H.264 sang MSE/WebRTC ngay từ đầu
...
[iPhone/iPad]
  ...
  |- WebDriverAgent H264     -> device port 8002
        ^
        | USB / tunnel / forward
        v
[Ubuntu]
...
3. Dự án này **không cần Appium server**, chỉ cần **WDA đang chạy được** và **port 8000/8001/8002 truy cập được từ Ubuntu**.
...
## Khi nào nên nâng cấp sang `quicktime_video_hack + ws-qvh`

Hãy chuyển sang `qvh/ws-qvh` nếu bạn cần:

- latency thấp hơn MJPEG
- H.264 thay vì chuỗi JPEG
- nhiều client và băng thông tối ưu hơn

Nhưng bản đó phức tạp hơn đáng kể trên Ubuntu vì bạn sẽ phải xử lý raw H.264 / remux / decode ở phía web.
