...
const REALTIME_PREFERRED_VIEW_MODES = ['mjpeg-wda', 'mjpeg-binary'];
const CONTROL_MODES_FALLBACK = [
  {id: 'realtime', label: 'Realtime socket', enabled: true, reachable: true},
  {id: 'auto', label: 'Auto fallback', enabled: true, reachable: true},
  {id: 'http', label: 'HTTP WDA', enabled: true, reachable: true},
];
...
const CONTROL_WAIT_FOR_SOCKET_MS = 260;
...
const el = {
  ...
  pointArrayEnabledInput: document.getElementById('pointArrayEnabledInput'),
...
function bindUi() {
  ...
  el.controlModeSelect.addEventListener('change', () => {
    const mode = el.controlModeSelect.value;
    el.controlModeLabel.textContent = mode || '-';
  ...
  el.viewer.addEventListener('pointerup', async (event) => {
    ...
    try {
      if (distance < TAP_THRESHOLD_PX) {
        const data = await sendTapGesture(end);
  ...
  ['pointercancel', 'pointerleave'].forEach((name) => {
    el.viewer.addEventListener(name, () => {
      if (state.dragging) {
        state.dragging = false;
        state.dragStart = null;
        resetDragTracking();
...
async function loadViewModes() {
  const hiddenWhenUnreachable = new Set(['scrcpy', 'webrtc-h264']);
  let viewData;
  try {
  ...
  const localLabel = localFallback?.enabled
    ...
    : '-';
  el.h264SourceLabel.textContent = upstream.h264WsUrl || upstream.h264TcpUrl || localLabel;
  el.scrcpySourceLabel.textContent = upstream.scrcpyWsUrl || upstream.h264TcpUrl || localLabel;
...
async function loadControlModes() {
  ...
  state.controlModes = modes.map((item) => ({
    id: String(item.id),
    label: String(item.label || item.id),
    enabled: item.enabled !== false,
    reachable: item.reachable !== false,
  ...
  el.controlModeLabel.textContent = preferredMode || '-';
  const upstream = state.controlStream?.upstream || {};
  const realtimeUrl = upstream.realtimeControlUrl || '-';
  if (preferredMode === 'http') {
    setControlState('http');
  } else if (upstream.realtimeReachable) {
    setControlState('socket available');
  } else if (preferredMode === 'auto') {
    setControlState('auto fallback');
  } else {
    setControlState('socket unavailable');
  }
...
function choosePreferredViewMode(selectedMode, modes) {
  ...
  const preferredRealtime = modes.find((item) =>
    REALTIME_PREFERRED_VIEW_MODES.includes(item.id) &&
    item.enabled &&
    item.reachable
  );
  if (preferredRealtime) {
    return preferredRealtime.id;
  }
...
function choosePreferredControlMode(selectedMode, modes) {
  ...
  const realtime = modes.find((item) =>
    item.id === 'realtime' &&
    item.enabled &&
    item.reachable
  );
  ...
  const auto = modes.find((item) => item.id === 'auto' && item.enabled);
  if (auto) {
    return auto.id;
  }

  return modes.find((item) => item.id === 'http' && item.enabled)?.id
    || modes.find((item) => item.enabled)?.id
    || modes[0]?.id
    || 'http';
}
...
function getRequestedControlMode() {
  try {
    ...
    return String(localStorage.getItem(CONTROL_MODE_STORAGE_KEY) || '').trim();
  } catch (err) {
    return '';
  }
}
...
function createMjpegBinaryRenderer() {
  const image = document.createElement('img');
  image.className = 'stream-media';
  image.alt = 'MJPEG binary websocket stream';
  image.decoding = 'async';
  ...
  image.addEventListener('load', () => {
    ...
    if (displayedObjectUrl && displayedObjectUrl !== loadingObjectUrl) {
      URL.revokeObjectURL(displayedObjectUrl);
    }
    displayedObjectUrl = loadingObjectUrl;
    loadingObjectUrl = '';
    frameLoading = false;
    fitMjpegSurfaceToViewer(image);
    pumpLatestFrame();
  ...
  image.addEventListener('error', () => {
    if (loadingObjectUrl) {
      URL.revokeObjectURL(loadingObjectUrl);
      loadingObjectUrl = '';
    }
    frameLoading = false;
    pumpLatestFrame();
  ...
  return {
    async start() {
      active = true;
      mountSurface(image, 'mjpeg-binary');
    ...
    async stop() {
      ...
      for (const url of [displayedObjectUrl, loadingObjectUrl, pendingObjectUrl]) {
        if (url) {
...
async function createBroadwayRenderer(mode, options = {}) {
  ...
  const endpoint = `${getEndpoint('h264Ws', '/ws/h264')}?source=${encodeURIComponent(requestedSource)}&stripScrcpyMeta=1`;
...
async function createScrcpyRenderer() {
  ...
  return {
    async start() {
      ...
      const endpoint = `${getEndpoint('h264Ws', '/ws/h264')}?source=scrcpy&stripScrcpyMeta=1`;
      socket = openAppWebSocket(endpoint);
      socket.binaryType = 'arraybuffer';
      socket.addEventListener('open', () => {
        setStreamState('connected (scrcpy)', true);
      });
      socket.addEventListener('message', (event) => {
        if (typeof event.data === 'string') {
          handleWsControl(event.data, 'scrcpy');
          return;
        }
        ...
        if (containsAnnexBNalType(packet, 5)) {
          hasKeyframe = true;
        }
        if (!hasKeyframe) {
          return;
        ...
        try {
          muxer.feed({video: packet, duration: 16});
          setStreamState('streaming (scrcpy)', true);
        } catch (err) {
          log(`scrcpy decode warning: ${err.message}`);
        }
      });
      socket.addEventListener('close', () => {
        setStreamState('scrcpy socket closed', false);
      });
      socket.addEventListener('error', () => {
        setStreamState('scrcpy socket error', false);
      });
    },
    async stop() {
      if (socket) {
...
function createTinyH264Renderer() {
  ...
  const endpoint = `${getEndpoint('h264Ws', '/ws/h264')}?source=h264&stripScrcpyMeta=1`;
  let socket = null;
  let worker = null;
...
function fitMjpegSurfaceToViewer(surfaceElement) {
  ...
  if (!['mjpeg-wda', 'mjpeg-binary'].includes(mode)) {
    return;
  }
...
function isPointArraySwipeEnabled() {
  return Boolean(el.pointArrayEnabledInput?.checked);
}

async function applySelectedControlMode(force = false) {
  const mode = el.controlModeSelect.value || state.selectedControlMode || 'http';
  ...
  const client = getRealtimeControlClient();
  try {
...
function getSelectedControlMode() {
  return el.controlModeSelect?.value || state.selectedControlMode || 'http';
}
...
async function sendTapGesture(point) {
  return sendControlCommand(
...
async function sendControlCommand(command, httpFallback) {
  ...
  try {
    const result = await client.send(command, {
      ...
      waitForOpenMs: mode === 'auto' ? CONTROL_WAIT_FOR_SOCKET_MS : CONTROL_COMMAND_TIMEOUT_MS,
    });
    setControlState('socket ready');
    return {
      ok: true,
...
function getRealtimeControlClient() {
  if (!state.controlClient) {
    const endpoint = getEndpoint('realtimeControlWs', '/ws/realtime-control');
...
function createRealtimeControlClient(endpoint) {
  ...
  const scheduleReconnect = () => {
    if (!shouldReconnect || reconnectTimer || getSelectedControlMode() !== 'realtime') {
      return;
    }
  ...
  return {
    ...
    async send(command, options = {}) {
      ...
      if (!socket || socket.readyState !== WebSocket.OPEN || !ready) {
        throw new Error('Realtime control socket is not ready');
      }
...
async function applySettings() {
  ...
  applySettingsInputs(data.sent || payload);
  ...
  if (state.selectedMode && !['mjpeg-wda', 'mjpeg-binary'].includes(state.selectedMode)) {
