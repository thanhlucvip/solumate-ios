...
Write-Host "Forwarding H264:  localhost:$HostH264Port -> device:$DeviceH264Port"
$p3 = Start-Process -FilePath "ios" -ArgumentList @("forward", "$HostH264Port", "$DeviceH264Port") -PassThru -NoNewWindow

$stdout = "Forwarding CTRL:  localhost:$HostRealtimeControlPort -> device:$DeviceRealtimeControlPort"
Write-Host $stdout
$p4 = Start-Process -FilePath "ios" -ArgumentList @("forward", "$HostRealtimeControlPort", "$DeviceRealtimeControlPort") -PassThru -NoNewWindow
