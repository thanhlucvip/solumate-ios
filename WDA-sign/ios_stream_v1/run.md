...
Ghi chu:
- Script da tu set `USE_PORT=8000`, `MJPEG_SERVER_PORT=8001`, `H264_SERVER_PORT=8002`, va `WDA_REALTIME_CONTROL_PORT=8003`.
...
### 5.1) (Tuy chon) Cau hinh mode H264/scrcpy/WebRTC

Mac dinh server se tu fallback MJPEG -> H264 binary websocket bang `ffmpeg` de chay:
- `Broadway`
- `h264-live-player`
- `tinyh264`
- `WebCodecs`
...
Neu ban co upstream H264 websocket rieng (vd scrcpy websocket server hoac ws-qvh), dat them env truoc khi `npm start`:

```powershell
$env:H264_WS_URL = "ws://127.0.0.1:27183"
$env:SCRCPY_WS_URL = "ws://127.0.0.1:27183"
$env:WEBRTC_WHEP_URL = "http://127.0.0.1:8889/whep"
$env:ALLOW_DYNAMIC_WS_SOURCE = "true"
npm start
```

Ghi chu:
- `H264_WS_URL` dung cho Broadway / h264-live-player / tinyh264.
- `SCRCPY_WS_URL` dung cho mode Genymobile/scrcpy (JMuxer).
- Neu `H264_WS_URL/SCRCPY_WS_URL` khong reachable, server se tu fallback local bridge neu `ffmpeg` co san.
...
3. Thu doi dropdown `View mode`:
   ...
   - `Broadway` / `h264-live-player` / `tinyh264` / `WebCodecs` / `Genymobile/scrcpy`: dung upstream H264 neu co, neu khong server se tu transcode MJPEG bang ffmpeg.
   - `H.264/WebRTC`: can co `WEBRTC_WHEP_URL`.

### 9.1) Trackpad trong man hinh chinh

- Trong panel ben phai co muc `Trackpad`:
  - `Use pointArray swipe`
  - `Sample interval (ms)`
- Khi bat `Use pointArray swipe`, thao tac keo trong khung stream se goi `POST /api/point-array`.
- Chon `Control mode = Realtime socket` de di qua `/ws/realtime-control` -> WDA TCP `8003`.
- Token `st` neu can se duoc server tu tao khi da cau hinh `SOLUMATE_WDA_SWIPE_SECRET`.
- Neu WDA khong co endpoint `/wda/swipe/pointArray`, UI se tu fallback sang `/api/swipe`.
...
netstat -ano | findstr :27184
netstat -ano | findstr :8889
```

Neu khong co process listen:
- Van co the chay `Broadway/h264-live-player/tinyh264/WebCodecs/Genymobile-scrcpy` neu local fallback ffmpeg dang hoat dong.
- Neu muon bat buoc dung upstream rieng thi kiem tra `H264_WS_URL` / `SCRCPY_WS_URL`.
- `H.264/WebRTC` can `WEBRTC_WHEP_URL` dang chay that.

Luu y quan trong:
- Voi iOS WDA thuong khong tu sinh H264 websocket. Ban can:
  - Co `ffmpeg` trong PATH (de local fallback transcode MJPEG -> H264), hoac
  - Tu cung cap upstream H264 websocket.
