...
function handleRealtimeControlWsClient(clientSocket, options = {}) {
  clientSocket._socket?.setNoDelay?.(true);
  const modeName = options.modeName || "realtime-control";
  const host = options.host || config.realtimeControlHost;
  const port = Number(options.port) || config.realtimeControlPort;
  const authToken =
  ...
  const decrementConnectionCount = () => {
    ...
    counted = false;
    state[connectionStateKey] = Math.max(
      0,
      (Number(state[connectionStateKey]) || 0) - 1,
    );
  };
...
function prepareRealtimeControlPayload(rawPayload, options = {}) {
...
function probeRealtimeControl(timeoutMs = 900, endpoint = {}) {
  const host = endpoint.host || config.realtimeControlHost;
  const port = Number(endpoint.port) || config.realtimeControlPort;
  const source = `tcp://${host}:${port}`;
  const authToken =
    endpoint.authToken !== undefined
      ? endpoint.authToken
      : config.realtimeControlAuthToken;
  const label = endpoint.label || "realtime-control";

  if (!host || !Number.isFinite(port) || port <= 0) {
    return Promise.resolve({
      reachable: false,
      warning: `Invalid ${label} endpoint: ${source}`,
    });
  }
  ...
  return new Promise((resolve) => {
    ...
    const handlePayload = (payload) => {
      ...
      if (authPending) {
        ...
        finalize(false, payload.error || `${label} authentication failed`);
        return;
      }
      ...
      if (probePending) {
        ...
        finalize(
          ...
          payload.error ||
            payload.message ||
            `Unexpected ${label} probe response: ${payload.type || "unknown"}`,
        );
        return;
      }
    ...
    socket.once("connect", () => {
      ...
      if (authToken) {
        ...
        if (!sendLine({type: "auth", token: authToken})) {
          finalize(false, `${label} auth could not be sent`);
        }
        return;
      }
      sendProbe();
    ...
    socket.once("error", (err) => {
      finalize(false, `Cannot connect ${label} socket ${source}: ${err.message}`);
    });
    socket.once("close", () => {
      if (!settled) {
        finalize(false, `${label} socket closed before probe completed`);
      }
    });
  });
}
